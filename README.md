...
実証：`--economy` で gemini は55件の取引が成立し、**「食料＝1.5マネー」という価格が実取引から
創発**（誰も設定していない）。LLM住民は経済を理解しているので、この物理の上でさらに豊かに
市場・分業・信用を回せます（次の素材＝信用/IOUを足せば、物々交換→貨幣の発生も観察可能）。
...
emergence/
  ...
  market.py       経済の物理（交換 OFFER/ACCEPT・生産 CRAFT・価格は創発）
