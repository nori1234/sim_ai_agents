...
> (`llm_model_agi`) を育て、接地を反証可能に測る測定器。GitHubリポ名は `sim_ai_agents`、
> パッケージ名は `emergence-world`（名称が別なだけ・役割は世界）。
> **全体像＝真実の単一ソースは [`docs/SYSTEM.md`](docs/SYSTEM.md)。**
...
チェックポイントが1つ成立**（＝再生では説明できない存在証明）。本番エンジン側では、
タスク自体が接地に報いていなかった欠陥（サンドボックスの預金チェーン）を測定で特定・修正し
（`sole_banker` 再設計、oracle優位 +0.56σ）、**初めて「接地が勝ち筋のタスク」での訓練を実施
——結果は POWERED-NO**（方策はレジーム非依存の平坦戦略に崩壊。`docs/runs/run-14/`）。
次の焦点は表現学習可能性（脳側）とタスクマージンの再事前登録。一括受け入れ検証は
`run_grounding_battery` で1コール化済み。
