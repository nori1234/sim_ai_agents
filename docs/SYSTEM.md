...
名前は据え置き、概念層でここに統一する。**実際のリポ名/パッケージ名のリネームは残課題として
[issue #194](https://github.com/nori1234/sim_ai_agents/issues/194) に分離**（CIのgit ref・adapter・
cross-link を壊す破壊的移行なので、接地調査#130が一段落してから対応）。
