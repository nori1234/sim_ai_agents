...
- #44 v2a+B2 C1: 密度崩壊（発火1.5–19%）、G1 −0.079（やや逆）。C1は設計通り反射不能→推論
  できず"預金を止めるだけ"。
- **⇒ (4)actuation は次の攻め所でない**（動かす信念が無い）。報酬整形/criticは時期尚早。
- **次＝A1**：recall でなく、(obs,行動,報酬/富デルタ) の**オンライン走る信念状態**を方策へ（`_priv`
  教師は継続、ただし*レジームを実際に運ぶ入力*の上で）。D1 で belief_acc>0.5 を再測＝(3)解決の直接判定。
  それが出て初めて (4)。**recall基盤で B2/critic 亜種を再走させない**（信号が無いと D1 が示した）。
