...
| `run-18` | [29786362493](https://github.com/nori1234/sim_ai_agents/actions/runs/29786362493) | **variance-reduction candidate 1** (`adv_baseline="episode"`, brain @435177b): the pre-declared criterion moves — deposit used-advantage separates by regime (control +0.278 vs cf +0.023; raw credit un-inverted, noise compressed ~10×), battery attempts flip to the correct order (248 C / 231 CF) — still POWERED-NO; run #19 = same spec, episodes 200 |
| `run-19` | [29789225282](https://github.com/nori1234/sim_ai_agents/actions/runs/29789225282) | **200 episodes on the episode baseline**: the pre-declared criterion essentially met on the signal side — deposit used-adv **cf −0.123 vs control −0.005** (middle third −0.229 vs +0.053) — but behaviour stayed regime-flat (propensity 0.081 vs 0.082; battery 359 C / 348 CF, still POWERED-NO). Tug-of-war visible: BC toward the regime-blind teacher (435 demos ≈ self 443) out-pulls the sparse correct PG signal → **the binding constraint moved to the teaching channel (issue #10 R2)**; next = BC annealing (run #20) |
| `run-20` | [29795755921](https://github.com/nori1234/sim_ai_agents/actions/runs/29795755921) | **BC annealing** (`bc_weight_decay_steps=300`, brain @a7f7ddc): the anneal fired (bc_weight 0.3→0.05) but grounding did not appear underneath — regime propensity separation stayed at noise (±0.002), deposit density fell as BC decayed, still POWERED-NO. **Self-play PG alone is too thin**; the blind teacher can bootstrap density but not contingency. Next: grounded scripted teacher (#10c) / exploration / memory integration |
